...
VRange::VRange(double min, double max, const std::string &minLabel, const std::string &maxLabel) : VRange(nullptr, min, max, minLabel, maxLabel) {}
...
void VRange::_adjustMaxToMin()
{
    ...
    if (low > high) _maxSlider->SetValue(low);
}
...
void VRange::_adjustMinToMax()
{
    double low = _minSlider->GetValue();
    double high = _maxSlider->GetValue();
    if (high < low) _minSlider->SetValue(high);
}
