...
int main(int argc, char **argv)
{
    ...
    voro::container con(ptcXMin, ptcXMax, ptcYMin, ptcYMax, ptcZMin, ptcZMax, 10, 10, 10, false, false, false, 100);
    long int        nPtcToUse = nParticles;    // use a subset of particles for experiments
    for (long int i = 0; i < nPtcToUse; i++) con.put(i, ptcX[i], ptcY[i], ptcZ[i]);
    ...
    for (long int i = 0; i < nPtcToUse; i++)
        if (gridPtCount[i] == 0) emptyCellCount++;
