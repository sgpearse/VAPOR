...
int main(int argc, char **argv)
{
    ...
    voro::container con(ptcXMin, ptcXMax, ptcYMin, ptcYMax, ptcZMin, ptcZMax, 10, 10, 10, true, true, true, 100);
    ...
    for (long int i = 0; i < nPtcToUse; i++) gridPtCount[i] = 0;
    ...
    for (long int z = 0; z < GRIDZ; z++) {
        ...
        for (long int y = 0; y < GRIDY; y++) {
            ...
            for (long int x = 0; x < GRIDX; x++) {
                ...
                double rx, ry, rz;
                int    pid;    // particle ID
                if (!con.find_voronoi_cell(gridX, gridY, gridZ, rx, ry, rz, pid)) std::cerr << "didn't find a voronoi cell!" << std::endl;
    ...
    for (long int i = 0; i < nPtcToUse; i++)
        if (gridPtCount[i] == 0) {
            ...
            long int lgrid[3];
            for (int j = 0; j < 3; j++) lgrid[j] = std::lround(grid[j]);
            density[lgrid[2] * GRIDX * GRIDY + lgrid[1] * GRIDY + lgrid[0]] += 1.0;

            std::cerr << "Found a Voronoi cell without a grid point: " << std::endl;
            for (int j = 0; j < 3; j++) std::cerr << "Particle coord = " << particle[j] << ";  Grid coord = " << lgrid[j] << std::endl;
        }
