...
public: 
 ...
 virtual ~AnnotationParams() {};
	...
	bool GetUseDomainFrame() const{
		return (0 != GetValueLong(_domainFrameTag, (long) false));
	}
	...
	bool GetUseRegionFrame() const{
		return (0 != GetValueLong(_regionFrameTag, (long) false));
	}
	...
	void GetTimeColor(T color[]) const {
		m_getColor(color, _timeColorTag);
	};
...
private:
	...
	template<typename T>
	void m_getColor(T color[3], string tag) const {
