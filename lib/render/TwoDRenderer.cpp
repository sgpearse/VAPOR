...
int TwoDRenderer::OSPRayUpdate(OSPModel world)
{
    ...
    for (int i = 0; i < W * H; i++) transformedData[i] = model * vec4(((vec3 *)_verts)[i], 1);
    ...
    for (int i = 0; i < W * H; i++) transformedData[i] = normalModel * ((vec3 *)_normals)[i];
...
#define I(x, y) (int)((y)*W + (x))
    ...
    for (int y = 0; y < CH; y++) {
        for (int x = 0; x < CW; x++) {
            if (_hasMissingData) {
                if (dataTex[I(x, y) * 2 + 1] != 0 || dataTex[I(x + 1, y) * 2 + 1] != 0 || dataTex[I(x + 1, y + 1) * 2 + 1] != 0 || dataTex[I(x, y + 1) * 2 + 1] != 0) continue;
            }
