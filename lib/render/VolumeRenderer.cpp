...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromRegularGrid(const Grid *grid, const glm::mat4 &transform)
{
    ...
    for (size_t i = 0; i < nVerts; ++i, ++dataIt) volumeData[i] = *dataIt;

    if (grid->HasMissingData()) {
        float missingValue = grid->GetMissingValue();
        for (size_t i = 0; i < nVerts; i++)
            if (volumeData[i] == missingValue) volumeData[i] = NAN;
    }
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromStructuredGrid(const Grid *grid, const glm::mat4 &transform)
{
    ...
    auto dataIt = grid->cbegin();
    for (size_t i = 0; i < nVerts; ++i, ++dataIt) scalarData[i] = *dataIt;
...
#define I(x, y, z) (int)((z)*VH * VW + (y)*VW + (x))
    ...
    for (int z = 0; z < VCD; z++) {
        for (int y = 0; y < VCH; y++) {
            for (int x = 0; x < VCW; x++) {
                if (hasMissing) {
                    if (scalarData[I(x, y, z)] == missingValue || scalarData[I(x + 1, y, z)] == missingValue || scalarData[I(x + 1, y + 1, z)] == missingValue
                        || scalarData[I(x, y + 1, z)] == missingValue || scalarData[I(x, y, z + 1)] == missingValue || scalarData[I(x + 1, y, z + 1)] == missingValue
                        || scalarData[I(x + 1, y + 1, z + 1)] == missingValue || scalarData[I(x, y + 1, z + 1)] == missingValue) {
                        continue;
                    }
                }
                indices[indexId++] = {
                    I(x, y, z), I(x + 1, y, z), I(x + 1, y + 1, z), I(x, y + 1, z), I(x, y, z + 1), I(x + 1, y, z + 1), I(x + 1, y + 1, z + 1), I(x, y + 1, z + 1),
                };
            }
        }
    }
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromUnstructuredGrid(const Grid *grid, const glm::mat4 &transform)
{
    ...
    for (size_t cellCounter = 0; cellCounter < nCells; ++cellIt, ++cellCounter) {
        ...
        if (numNodes == 4) {
            ...
            OSPRayCell c;
            for (int i = 0; i < 8; i++) c.i[i] = nodes[i * nodeDim] + nodes[i * nodeDim + 1] * nodeDims[0];
        ...
        if (hasMissing) {
            ...
            for (int i = 0; i < 8; i++)
                if (cells[cellId - 1].i[i] >= 0 && scalarData[cells[cellId - 1].i[i]] == missingValue) cellHasMissing = true;
            if (cellHasMissing) cellId--;
        }
    }
    delete[] scalarData;

    for (int i = 0; i < 8; i++) { printf("%i nodes = %i\n", i + 1, n[i]); }
