...
int DCHDF5::initialize(const vector<string> &paths, const std::vector<string> &options)
{
...
bool DCHDF5::getDimension(string dimname, DC::Dimension &dimension) const
{
...
bool DCHDF5::getMesh(string mesh_name, DC::Mesh &mesh) const
{
...
template<class T> bool DCHDF5::_getAttTemplate(string varname, string attname, T &values) const
{
    DC::BaseVar var;
...
bool DCHDF5::getAtt(string varname, string attname, vector<double> &values) const
{
    values.clear();
...
bool DCHDF5::getAtt(string varname, string attname, vector<long> &values) const
{
    values.clear();
...
bool DCHDF5::getAtt(string varname, string attname, string &values) const
{
    values.clear();
...
int DCHDF5::getDimLensAtLevel(string varname, int, std::vector<size_t> &dims_at_level, std::vector<size_t> &bs_at_level) const
{
    dims_at_level.clear();
    bs_at_level.clear();
...
int DCHDF5::openVariableRead(size_t ts, string varname)
{
...
template<class T> int DCHDF5::_readRegionTemplate(int fd, const vector<size_t> &min, const vector<size_t> &max, T *region)
{
...
bool DCHDF5::variableExists(size_t ts, string varname, int, int) const { return (_ncdfc->VariableExists(ts, varname)); }

int DCHDF5::_get_latlon_coordvars(NetCDFCFCollection *ncdfc, string dvar, string &loncvar, string &latcvar) const
{
    loncvar.clear();
    latcvar.clear();
...
int DCHDF5::_AddCoordvars(NetCDFCFCollection *ncdfc, const vector<string> &cvars)
{
    for (int i = 0; i < cvars.size(); i++) {
        // Get dimension names
        //
        ...
        _coordVarsMap[cvars[i]] = CoordVar(cvars[i], units, DC::FLOAT, periodic, axis, false, dimnames, time_dim_name);
...
int DCHDF5::_InitHorizontalCoordinates(NetCDFCFCollection *ncdfc)
{
    //
...
int DCHDF5::_get_vertical_coordvar(NetCDFCFCollection *ncdfc, string dvar, string &cvar)
{
    cvar.clear();
...
//
//
int DCHDF5::_InitVerticalCoordinates(NetCDFCFCollection *ncdfc)
{
    //
...
int DCHDF5::_get_time_coordvar(NetCDFCFCollection *ncdfc, string dvar, string &cvar)
{
    cvar.clear();
...
int DCHDF5::_InitTimeCoordinates(NetCDFCFCollection *ncdfc)
{
    //
    // Get names of data variables  in the CF data set that have 1, 2 or 3
    // spatial dimensions
...
// _dimsMap
//
int DCHDF5::_InitDimensions(NetCDFCFCollection *ncdfc)
{
    _dimsMap.clear();
...
// is significant.
//
int DCHDF5::_GetVarCoordinates(NetCDFCFCollection *ncdfc, string varname, vector<string> &sdimnames, vector<string> &scoordvars, string &time_dim_name, string &time_coordvar)
{
    sdimnames.clear();
    scoordvars.clear();
    time_dim_name.clear();
